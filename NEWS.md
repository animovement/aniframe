...
* `add_events()` now uses the per-row `type` column on the anievent to classify each bout. For each `(channel, type)` group, one factor column is emitted on the aniframe — bare `<channel>` when the channel carries only one type, suffixed `<channel>_state` / `<channel>_point` when it carries both. Track-splitting on overlap still produces `<channel>_2` / `<channel>_state_2` etc. as a nested suffix. The previous `variables_event` argument is no longer needed (the per-row classification subsumes it) and has been removed.
* `as_anievent.aniframe()` round-trips both suffix conventions: `_state` / `_point` strips back to the base channel name with the appropriate `type` per row, and the existing `_<n>` track suffix bundling still works (and composes — `behaviour_state_2` strips both ways to recover channel `behaviour` with type `state`).
