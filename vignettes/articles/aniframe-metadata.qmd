...
`set_metadata()` therefore refuses all four, and points you at the
...
Each role has the same four verbs as `connections`:
`get_variables_*()`, `set_variables_*()`, `add_variables_*()` and
`remove_variables_*()`. The column has to exist before it can be
declared, so the order is always create-then-declare:
