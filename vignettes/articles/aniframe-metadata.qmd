...
| **Slot vocabulary**| `variables_what`, `variables_when`, `variables_index`, `variables_where` |
...
`variables_what`, `variables_when`, `variables_index`, `variables_where`
and `variables_event` are a special case: they name columns rather than
describing values, so a name that matches nothing is a promise the frame
can't keep. All but `variables_event` go further — they are not a
...
`set_metadata()` therefore refuses all five, and points you at the
