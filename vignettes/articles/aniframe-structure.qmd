...
|                     | Index (`variables_index`)   | A position within that context | `time` |
| **Where**?          | Spatial (`variables_where`) | A position  | `x`, `y`                 |

The **when** slot has two halves, because "when" is two questions: which
recording session this row belongs to, and where it sits inside it. The
first is context the row shares with its neighbours; the second is what
tells it apart from them. They behave differently enough — one groups,
the other orders — that they are declared separately.
...
monotonically ordered within each entity, which is how `as_aniframe()`
arranges the frame.
