...
Both take vectors so multiple edges go in or out in one call:
...
fish <- remove_connections(fish, from = "head", to = "neck")
nrow(get_connections(fish, "keypoint"))
```

`remove_connections()` matches exactly on `from`/`to` — call it twice
with swapped arguments if you want to remove both directions.
