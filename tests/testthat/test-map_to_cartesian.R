...
test_that("map_to_cartesian_*() works with empty data frames (zero rows)", {
  empty_polar <- dplyr::tibble(
    ...
    as_aniframe()
  empty_cyl   <- dplyr::tibble(keypoint = integer(),
                                time = numeric(),
                                rho = numeric(),
                                phi = numeric(),
                                z   = numeric()) |>
    as_aniframe()
  empty_sph   <- dplyr::tibble(keypoint = integer(),
                                time = as.numeric(),
                                rho = numeric(),
                                phi = numeric(),
                                theta = numeric()) |>
...
test_that("map_to_cartesian_*() preserves additional non‑coordinate columns", {
  df_extra <- make_polar_df() |> as_aniframe() |>
...
test_that("Cartesian results from the three systems are mutually consistent when the inputs represent the same point", {
  ...
  # Polar (no z)
  df_pol <- dplyr::tibble(keypoint = 1, time = 1, rho = rho_xy, phi = phi_xy) |> as_aniframe()
  # Cylindrical (adds the known z)
  df_cyl <- dplyr::tibble(keypoint = 1, time = 1, rho = rho_xy, phi = phi_xy, z = z) |> as_aniframe()
  # Spherical (uses theta)
  df_sph <- dplyr::tibble(keypoint = 1, time = 1, rho = rho_xy, phi = phi_xy, theta = theta_sp) |> as_aniframe()
