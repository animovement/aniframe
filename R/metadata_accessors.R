...
#' Where the coordinate origin sits
#'
#' @param data An aniframe or anievent object.
#'
#' @return Length-one character vector: `"bottom_left"`, `"top_left"` or
#'   `"none"`.
#'
#' @examples
#' af <- example_aniframe(n_obs = 3, n_individuals = 1, n_keypoints = 1)
...
get_origin <- function(data) {
  ensure_is_aniframe_or_anievent(data)
  as.character(get_metadata(data, "origin"))
}


#' The height of the recording frame, in y-axis units
#'
#' The extent [set_origin()] reflects around when switching convention.
#'
#' @param data An aniframe or anievent object.
#'
#' @return Numeric scalar, or `NA` when it is not recorded.
#'
#' @examples
#' af <- example_aniframe(n_obs = 3, n_individuals = 1, n_keypoints = 1)
#' get_y_height(af)
...
#' @export
get_y_height <- function(data) {
  ensure_is_aniframe_or_anievent(data)
  get_metadata(data, "y_height")
}
