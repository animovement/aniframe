  ...
  if (!is.null(to_keypoint)) {
    if (is.character(to_keypoint)) {
      if (length(to_keypoint) > 1) {
        cli::cli_abort(
          "Only 1 keypoint can be supplied to `to_keypoint`, received {length(to_keypoint)}."
        )
      }
      if (!to_keypoint %in% unique(data$keypoint)) {
        cli::cli_abort(
          "Keypoint {to_keypoint} is not among the keypoints in the data, which are {unique(data$keypoint)}."
        )
      }
    ...
    if (!all(is.numeric(to_x) & is.numeric(to_y))) {
      cli::cli_abort(
        "Expected to_x and to_y must be numeric. Got {class(to_x)} and {class(to_y)}."
      )
    }
